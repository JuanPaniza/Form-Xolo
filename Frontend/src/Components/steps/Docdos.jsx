...
function Docdos({handleChange, handleKeyUp, checkState, inputRealTimeValidation, handleClick, errorForm}) {
  const questionsDos = [
    {
      id: 1,
      ...
      inputText: { value: "MedicationInput", label: "Mention what they are?" },
    },
    {
      id: 2,
      ...
      inputText: { value: "AcneMedicCincoInput", label: "Mention what they are?" },
    },
    {
      id: 3,
      ...
      inputText: { value: "MedicalDeviceInput", label: "Mention what they are?"},
    },
  ];
 
  return (
    <div>
      {questionsDos.map((question) => (
        <div key={question.id}>
          <div className="mt-7">
              ...
              <div className="flex flex-col gap-2 ">
                <label className={`${checkState[question.name] === "yes" ? "block mt-3 text-sm text-gray-700 text-center font-semibold": "block mt-3 text-sm text-gray-400 text-center font-semibold"}`}>
                  ...
                  <input 
                    ...
                    className={`${inputRealTimeValidation[question.inputText.value] === false && checkState[question.name] == "yes" ? " mt-5 focus:outline-none focus:border-red-300 border-2 border-solid border-red-500 redondeado-lg bg-red-100  w-full h-10 px-3 mb-0 rounded-md": "rounded-lg border w-full h-10 px-3 mb-5 focus:outline-none focus:border-gray-400  mt-5"}`}/>
      ...
      <div className="container flex justify-around pt-12  mb-8">
        ...
        <button onClick={() =>handleClick("next")}   type="button" className=" bg-gray-500 text-white uppercase py-2 px-4 rounded-xl font-semibold cursor-pointer hover:bg-slate-700 hover:text-white transition duration-200 ease-in-out hover:shadow-inner focus:outline-none transform hover:-translate-x hover:scale-105 ">
          Next
        </button>
      </div>
    </div>
